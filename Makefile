...
LINUX_ARM64_BIN := ./kiberamd-linux-arm64
...
## ローカルビルド済みバイナリを指定EC2 (via SSM) の /root/kiberamd に配置
deploy-ec2: build-linux-arm64
	...
	@tmp_url=$$(curl -s -F "reqtype=fileupload" -F "fileToUpload=@$(LINUX_ARM64_BIN)" https://catbox.moe/user/api.php); \
	 ...
	 backup_ts=$$(date +%s); \
	 payload=$$(TMP_URL="$$tmp_url" INSTANCE_ID="$(INSTANCE_ID)" BACKUP_TS="$$backup_ts" python3 -c "import json, os;url=os.environ['TMP_URL'].strip();instance_id=os.environ['INSTANCE_ID'].strip();backup=os.environ['BACKUP_TS'].strip();command=('bash -lc \"set -euo pipefail; if [ -f /root/kiberamd ]; then mv /root/kiberamd /root/kiberamd.bak.{backup}; fi; curl -fL {url!r} -o /root/kiberamd; chmod +x /root/kiberamd\"').format(backup=backup, url=url);payload={'DocumentName':'AWS-RunShellScript','Targets':[{'Key':'instanceids','Values':[instance_id]}],'Comment':'deploy kiberamd binary','Parameters':{'commands':[command]}};print(json.dumps(payload))"); \
